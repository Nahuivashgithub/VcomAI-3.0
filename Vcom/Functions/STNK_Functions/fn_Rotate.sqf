...
private ["_position", "_pos_x_new", "_pos_y_new", "_face_angle", "_diff_angle"];

params ["_tank", "_face"];

private _max_angle = 140;
private _min_angle = 20;
private _step = 20; // step to iterate from maximum angle to minimum
private _dist = 10; // Minimal move distance the tank will react to is 10 meters.
private _th = 20; // Threshold. If current diff is less that _th, don't bother to turn
private _err = 5; // accuracy -- how precisely we should turn
private _max_tries = 10; // how many iterations to rotate the tank we have
private _tank_pos = [];
private _pos_x = 0;
private _pos_y = 0;
...
// Need this if we loop AL9K_fnc_Setup more frequently (1 second instead of 5).
// Emulate lag between spotting and rotating.
sleep 2.5;

private _diff = [_tank, _face] call AL9K_fnc_Calc_Diff;
if ( abs _diff > _th and currentCommand _tank != "MOVE" and speed _tank < 1) then 
{
	private _tries = 0;
		...
		if (_tries > 0) then {
			...
			if (count _position == 0) then {
				...
				if (speed _tank < 1) then {
					...
					_face_angle = [getPosASL _tank, _position] call AL9K_fnc_Calc_Direction; //direction of move order
					while { (_countDown > 0) and (abs _diff > _err) and 
					(abs _diff_angle > 10) } do {
						_diff = [_tank, _face] call AL9K_fnc_Calc_Diff;
						_diff_angle = [_tank, _face_angle] call AL9K_fnc_Calc_Diff;
