...
diag_log format ["AL9K_fnc_Setup: started for %1, isServer: %2", _unit, str isServer];

// Do we have tracks?
{
	if (_unit isKindOf _x) exitWith {_has_tracks = true;};
...
if (_ap_round != "" and _has_cannon) then {
	...
	[_unit, _ap_round] call AL9K_fnc_Load_Mag;
};

if VCM_DEBUG then {[_unit, "Start"] call AL9K_fnc_LogTxt;};

// Main loop start
while { alive _unit and local _unit} do {
	
	if (
		(!isPlayer commander _unit) &&
		{!isPlayer gunner _unit} &&
		{!isPlayer driver _unit} &&
		{!isNull commander _unit || !isNull gunner _unit} &&
		{behaviour (effectiveCommander _unit) == "COMBAT" || behaviour (effectiveCommander _unit) == "AWARE" || behaviour (effectiveCommander _unit) == "STEALTH"} && 
		{currentCommand _unit == "" || currentCommand _unit == "STOP" || currentCommand _unit == "MOVE"}
	) then 
	{
		
		// A workaround for the bug when the commander is not effective commander
		// Occurs for Panther, Marshal etc
		if (!isNull commander _unit and (commander _unit != effectiveCommander _unit)) then 
		{
		...
		if (count _neartargets > 0) then {
			...
			if (_has_cannon) then {
				if (_he_round != "" and ([_unit, _he_round] call AL9K_fnc_Has_Ammo > 0)) then 
				{
					_has_he = true;
				}
				else 
				...
				if (_heat_round != "" and ([_unit, _heat_round] call AL9K_fnc_Has_Ammo > 0)) then
				{
					_has_heat = true;
				}
				else 
				...
				if (_ap_round != "" and ([_unit, _ap_round] call AL9K_fnc_Has_Ammo > 0)) then 
				{
					_has_ap = true;
				}
				else 
		...
		// Target identification
			  ...
			  
			{
				...
				if ((_x select 3) > 100) then
				{
					_threat_array = [_x, _unit] call AL9K_fnc_Calc_Threat;
					...
					_los = [_x, _unit] call AL9K_fnc_Calc_Los;
					
					
					// Find most dangerous target
					if ((_threat > _max_threat) and (_threat > 100) and _los) then 
					{
						_max_threat = _threat;
						_target = _x select 4;
						_type = _x select 1;
					...
					if (_has_cannon and _los and _can_fire) then 
					{
						...
						if (_value > 100) then 
						{
							...
							if ((_value > _max_thr_fire) and (_value > 100)) then
							{
		...
		// Actions.
			...
			if (_has_cannon) then 
			{
				...
				if (currentWeapon _unit == _cannon_name) then 
				{
					if (_current_mag != currentMagazine _unit) then 
					{
						...
						if VCM_DEBUG then {[_unit, "Changed default round to " + _current_mag] call AL9K_fnc_LogTxt;};
					};
				};
				if (!isNull _target_act) then 
				{
					...
					{
						// [_unit, format["Target: %1", _target_act]] call AL9K_fnc_LogTxt;
					};*/
					_last_aim = time;
					switch (_armor_aimed) do 
					{
						case 0: 
						{
							...
							if (_value_aimed > 100) then 
							{
								...
								if (_dist_aimed > 300) then 
								{
									if (_has_he) then 
									{
										...
										{
											if VCM_DEBUG then {[_unit, "Loading " + _he_round] call AL9K_fnc_LogTxt;};
											[_unit, _he_round] call AL9K_fnc_Load_Mag;
											_current_mag = _he_round;
										};
										if (currentWeapon _unit != _cannon_name) then 
										{
											if VCM_DEBUG then {[_unit, "Switching to cannon"] call AL9K_fnc_LogTxt;};
								...
								else 
								{
									...
									{
										if VCM_DEBUG then {[_unit, "Switching to MG"] call AL9K_fnc_LogTxt;};
							...
							else
							{
								if (_current_mag != _default_mag) then 
								{
									...
									{
										if VCM_DEBUG then {[_unit, "Loading default: " + _default_mag] call AL9K_fnc_LogTxt;};
										[_unit, _default_mag] call AL9K_fnc_Load_Mag;
										_current_mag = _default_mag;
									};
								};
								// If default is HE, then we mean to fire
								// cannon at every single infantry
								if (_default_mag == _he_round && _has_he && _dist_aimed > 300) then 
								{
									if (currentWeapon _unit != _cannon_name) then 
									{
										if VCM_DEBUG then {[_unit, "Switching to cannon"] call AL9K_fnc_LogTxt;};
						...
						case 1: 
						{
							if (_has_heat and _current_mag != _heat_round) exitWith 
							{
								if VCM_DEBUG then {[_unit, "Loading " + _heat_round] call AL9K_fnc_LogTxt;};
								[_unit, _heat_round] call AL9K_fnc_Load_Mag;
							...
							if (_has_ap and _current_mag != _ap_round) exitWith 
							{
								if VCM_DEBUG then {[_unit, "Loading " + _ap_round] call AL9K_fnc_LogTxt;};
								[_unit, _ap_round] call AL9K_fnc_Load_Mag;
								_current_mag = _ap_round;
								if (currentWeapon _unit != _cannon_name) then 
								{
			...
			else 
			{
				...
				if (_has_cannon and (time - _last_aim) > 30) then 
				{
					if (_current_mag != _default_mag) then 
					{
						if VCM_DEBUG then {[_unit, "No targets, loading " + _default_mag] call AL9K_fnc_LogTxt;};
						[_unit, _default_mag] call AL9K_fnc_Load_Mag;
						_current_mag = _default_mag;
					};
					if (currentWeapon _unit != _cannon_name) then {
						if VCM_DEBUG then {[_unit, "No targets, switching to cannon"] call AL9K_fnc_LogTxt;};
						_unit selectWeapon _cannon_name;
					};
				};
				if (scriptDone _handle_scan) then 
				{
					_handle_scan = [_unit] spawn AL9K_fnc_Scan;
				};
			};
			...
			{
				// direction to target
				_direction = [getPosASL _unit, _known_pos] call AL9K_fnc_Calc_Direction;

				if (
					currentCommand _unit == "MOVE" && 
					{behaviour (effectiveCommander _unit) != "STEALTH"} && 
					{_max_threat > 500} &&
					{_has_tracks} && 
					{scriptDone _handle_rotate}
				) then 
				{
					_was_moving = true;
				...
				if (_has_tracks and !isNull driver _unit) then 
				{
					...
					) then 
					{
						...
						if (scriptDone _handle_rotate) then {
							_handle_rotate = [_unit, _direction] spawn AL9K_fnc_Rotate;
						};
					};
				};
			}
...
if VCM_DEBUG then {[_unit, "End"] call AL9K_fnc_LogTxt;};